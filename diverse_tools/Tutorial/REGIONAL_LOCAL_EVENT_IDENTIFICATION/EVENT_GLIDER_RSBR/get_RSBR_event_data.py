'''
--------------------------------------------------------------------------------
 Function to download data from RSBR
--------------------------------------------------------------------------------
...
STA_LOC_PATH = '/media/sysop/14f7ead0-5dcb-4557-a139-55dbb404d11a/diogoloc/dados_posdoc/Gliders_DATA/STA_COORD/estacoes_RSBR_costa.txt'

# Event list file path
EV_LOC_PATH = '/media/sysop/14f7ead0-5dcb-4557-a139-55dbb404d11a/diogoloc/dados_posdoc/Gliders_DATA/EVENTS/CMTSOLUTIONS.xml'

#Directory to save EVENT data
OUTPUT_EV_DIR = '/media/sysop/14f7ead0-5dcb-4557-a139-55dbb404d11a/diogoloc/dados_posdoc/Gliders_DATA/EVENTS_RSBR_DATA/'
...
def cut_data_by_event(input):
    try:
        ...
        arrivals = model.get_travel_times(source_depth_in_km=ev_depth, distance_in_degree=gcarc, phase_list=["P","PKP","PKIKP"])
        #try:
        arr = arrivals[0]
        ...
        client = Client("USP")
        st = client.get_waveforms(knetwk, kstnm, "*", "*", starttime, endtime)
        ...
        st.write(event_directory+'/'+knetwk+'.'+kstnm+'.'+'{:04}'.format(op.UTCDateTime(ev_timeUTC).year)+'.'+'{:03}'.format(op.UTCDateTime(ev_timeUTC).julday)+'.'+'{:02}'.format(op.UTCDateTime(ev_timeUTC).hour)+'.'+'{:02}'.format(op.UTCDateTime(ev_timeUTC).minute)+'.'+'{:02}'.format(op.UTCDateTime(ev_timeUTC).second)+'.'+'{:02}'.format(op.UTCDateTime(ev_timeUTC).microsecond)[:3], format='MSEED')
    except:
        pass
# ============================================
...
events_file = read_events(EV_LOC_PATH)
for ev in events_file:
    ev_timeUTC.append(ev.origins[0].time.strftime("%Y-%m-%dT%H:%M:%S"))
    evla.append(ev.origins[0].latitude)
    evlo.append(ev.origins[0].longitude)
    evdp.append(ev.origins[0].depth/1000)
